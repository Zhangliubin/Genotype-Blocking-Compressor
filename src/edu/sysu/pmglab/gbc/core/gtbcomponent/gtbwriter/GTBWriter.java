...
public class GTBWriter implements AutoCloseable {
    ...
    final Array<GTBNode> GTBNodeCache = new Array<>(1024, true);
    ...
    public boolean write(Variant info) throws IOException {
        try {
            ...
            if (this.alleleQC.size() != 0) {
                ...
                // 单倍型
                if (ChromosomeTags.getPloidy(ChromosomeTags.getIndex(info.chromosome)) == 1) {
            ...
            // 将这个位点写入缓冲
            int currentVariantChromosome = ChromosomeTags.getIndex(info.chromosome);
