...
public class ManagerStringBuilder {
    ...
    ManagerStringBuilder(GTBManager manager) {
        this.manager = manager;
        add("fileName", "GTB File Name: " + manager.getFileName());

        // 记录文件大小
        add("fileSize", "GTB File Size: " + FileUtils.sizeTransformer(manager.getFileSize(), 3));
    ...
    public ManagerStringBuilder listFileBaseInfo(boolean enable) {
        ...
        if (enable) {
            add("originSize", "(Estimated) Decompressed Block Size: <=" + FileUtils.sizeTransformer(info.getEstimateDecompressedBlockSize(), info.getEstimateDecompressedBlockSizeFlag() == 15 ? 2 : 0));
    ...
    public ManagerStringBuilder calculateMd5(boolean enable) throws IOException {
        if (enable) {
            add("md5", "MD5 Code: " + MD5.check(this.manager.getFileName()));
        } else {
     ...
     * 列出 GTB 树，并指定染色体编号
     */
    public ManagerStringBuilder listGTBTree(boolean enable, int[] chromosomeIndexes) {
        if (enable) {
            add("gtbNodes", "\nSummary of GTB Nodes:\n" + this.manager.getGtbTree().nodeInfo(chromosomeIndexes));
        } else {
            if (this.builder.containsKey("gtbNodes")) {
                remove("gtbNodes");
            }
    ...
    public ManagerStringBuilder listGTBTree(boolean enable, String[] chromosomes) {
        return listGTBTree(enable, ChromosomeTags.getIndexes(chromosomes));
    }
    ...
    public ManagerStringBuilder listChromosomeInfo(boolean enable, int[] chromosomeIndexes) {
        if (enable) {
            add("gtbNodes", "\nSummary of GTB Nodes:\n" + this.manager.getGtbTree().chromosomeInfo(chromosomeIndexes));
        } else {
            if (this.builder.containsKey("gtbNodes")) {
                remove("gtbNodes");
            }
    ...
    public ManagerStringBuilder listChromosomeInfo(boolean enable, String[] chromosomes) {
        return listChromosomeInfo(enable, ChromosomeTags.getIndexes(chromosomes));
    }
