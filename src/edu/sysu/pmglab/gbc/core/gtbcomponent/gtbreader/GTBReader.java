...
public class GTBReader implements Closeable, AutoCloseable, Iterable<Variant> {
    ...
    public void limit(String chromosome, int startNodeIndex, int endNodeIndex) {
        ...
        this.pointer = new LimitPointer(this.manager, new int[]{ChromosomeTags.getIndex(chromosome)}, startNodeIndex, endNodeIndex);
    }
    ...
    public void limit(int chromosomeIndex, int startNodeIndex, int endNodeIndex) {
        Assert.that(manager.contain(chromosomeIndex), GBCExceptionOptions.GTBComponentException, "chromosome=" + ChromosomeTags.getString(chromosomeIndex) + " (index=" + chromosomeIndex + ") not in current GTB file");
    ...
    public Variant readVariant(Map<?, Set<Integer>> condition) throws IOException {
        ...
        do {
            ...
            if (!status) {
                String chromosome = ChromosomeTags.getString(node.chromosomeIndex);
    ...
    public Variant[] readVariants() throws IOException {
        ...
        if (variant != null) {
            TaskVariant taskVariant;
            Array<Variant> variants = new Array<>();
            variants.add(variant);
            int chromosomeIndex = ChromosomeTags.getIndex(variant.chromosome);
    ...
    public boolean readVariant(Variant variant) throws IOException {
        if (pointer.chromosomeIndex != -1) {
            ...
            variant.chromosome = ChromosomeTags.getString(node.chromosomeIndex);
            variant.position = taskVariant.position;
            variant.ploidy = ChromosomeTags.getPloidy(node.chromosomeIndex);
     ...
     * 从 variantCache 中获取位点，并将结果放入另一区域
     */
    public Array<Variant> readVariants(BaseArray<Variant> variantCache, Map<?, Set<Integer>> condition) throws IOException {
        if (condition == null) {
            return readVariants(variantCache);
        }
    ...
    public boolean seek(String chromosome, int position) throws IOException {
        return seek(ChromosomeTags.getIndex(chromosome), position);
    }
    ...
    public <SubjectType> void selectSubjects(BaseArray<SubjectType> subjects) {
