...
import edu.sysu.pmglab.gbc.core.build.BuildTask;
...
enum BuildFunction {
    ...
    private final CommandParser parser = BuildParser.getParser();
    ...
    public static int submit(String... args) throws IOException {
        ...
        if (options.isPassedIn("-h")) {
            ...
            // 预处理工作
            BuildTask task = (BuildTask) new BuildTask((String[]) options.get("build"))
            ...
            // 设置质控参数
            if (options.isPassedIn("--no-qc")) {
                task.controlDisable();
            } else {
                // 质量控制
                task.setGenotypeQualityControlDp((int) options.get("--gty-dp"))
                        .setGenotypeQualityControlGq((int) options.get("--gty-gq"))
                        .setVariantPhredQualityScore((int) options.get("--seq-qual"))
                        .setVariantQualityControlDp((int) options.get("--seq-dp"))
                        .setVariantQualityControlMq((int) options.get("--seq-mq"))
                        .setVariantQualityControlAllelesNum((int) options.get("--max-allele"));
