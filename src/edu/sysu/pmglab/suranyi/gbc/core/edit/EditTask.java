...
public class EditTask implements ITask {
    ...
    private final SmartList<String> mergeFiles = new SmartList<>(10, true);
    ...
    public <ChromosomeType> EditTask delete(Collection<ChromosomeType> chromosomes) {
        return delete(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public <ChromosomeType> EditTask retain(Collection<ChromosomeType> chromosomes) {
        return retain(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public EditTask concat(String... otherInputFileNames) {
        return concat(new SmartList<>(otherInputFileNames));
    }
    ...
    public EditTask concat(GTBManager... otherManagers) {
        SmartList<String> otherInputFileNames = new SmartList<>(otherManagers.length);
     ...
     */
    public <ManagerType> EditTask concat(Collection<ManagerType> otherUnknownTypeManagers) {
        Assert.NotNull(otherUnknownTypeManagers);

        if (otherUnknownTypeManagers.size() > 0) {
            SmartList<String> managerFileNames = new SmartList<>(otherUnknownTypeManagers.size());
            ...
            managerFileNames.removeAll(mainManager.getFileName());
            ...
            if (operators.size() > 0) {
                ...
                if (operators.contains(Operator.SPLIT)) {
                    for (int chromosomeIndex : mainManager.getChromosomeList()) {
                        ...
                        outs.put(chromosomeIndex, new FileStream(this.outputFileName + "/chr" + ChromosomeInfo.getString(chromosomeIndex) + ".gtb", FileOptions.CHANNEL_WRITER));
