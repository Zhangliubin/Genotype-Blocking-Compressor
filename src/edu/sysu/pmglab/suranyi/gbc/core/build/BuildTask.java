...
import edu.sysu.pmglab.suranyi.gbc.core.common.qualitycontrol.genotype.IGenotypeQC;
import edu.sysu.pmglab.suranyi.gbc.core.common.qualitycontrol.variant.*;
...
public class BuildTask extends IBuildTask {
    final SmartList<String> inputFileNames;
     ...
     * @param outputFileName 输出文件名，只能是单个文件名
     */
    public BuildTask(Collection<String> inputFileNames, String outputFileName) throws IOException {
        // 非空且非长度非 0
        ...
        if (files.length == 1) {
            this.inputFileNames = new SmartList<>(files);
        } else {
            // 长度不为 1, 则此时筛选扩展名为 vcf 和 gz 的文件 (传入多个文件或者文件夹时，需要遍历其中的子文件)
            this.inputFileNames = new SmartList<>(FileUtils.filterByExtension(files, ".vcf", ".gz"));
     ...
     * 获取输入文件名
     */
    public SmartList<String> getInputFileNames() {
