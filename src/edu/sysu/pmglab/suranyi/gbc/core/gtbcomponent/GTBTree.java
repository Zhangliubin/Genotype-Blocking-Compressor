...
public class GTBTree implements Iterable<GTBNodes> {
     ...
     */
    public GTBTree() {
        this(ChromosomeInfo.supportedChromosomeList().length);
    }
     ...
     * 构造器方法
     */
    public GTBTree(Collection<GTBNode> gtbNodes) {
        this(24);
    ...
    public GTBNodes get(String chromosome) {
        return get(ChromosomeInfo.getIndex(chromosome));
    }
    ...
    public GTBNodes[] get(String... chromosomes) {
        return get(ChromosomeInfo.getIndexes(chromosomes));
    }
    ...
    public <ChromosomeType> GTBNodes[] get(Collection<ChromosomeType> chromosomes) {
        return get(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public void resetContig(String resourceName) throws IOException {
        // 资源文件名相同时不进行转换
        synchronized (this) {
            if (resourceName != null && !(ChromosomeInfo.getCurrentFile().equals(resourceName))) {
                // 打开文件资源
                ...
                SmartList<String> chromosomes = new SmartList<>();
                ...
                for (int chromosomeInd : this.rootNodes.keySet()) {
                    int newIndex = chromosomes.indexOf(ChromosomeInfo.getString(chromosomeInd));
                    if (newIndex != -1) {
            ...
            // 加载新的资源文件
            ChromosomeInfo.load(resourceName);
        }
    }
    ...
    public boolean contain(String chromosome) {
        return contain(ChromosomeInfo.getIndex(chromosome));
    }
    ...
    public boolean[] contain(String... chromosomes) {
        return contain(ChromosomeInfo.getIndexes(chromosomes));
    }
    ...
    public <ChromosomeType> boolean[] contain(Collection<ChromosomeType> chromosomes) {
        return contain(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public <ChromosomeType> boolean containAll(Collection<ChromosomeType> chromosomes) {
        return containAll(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public void remove(int... chromosomeIndexes) {
        for (int chromosomeIndex : chromosomeIndexes) {
            Assert.that(contain(chromosomeIndex), GBCExceptionOptions.GTBComponentException, "chromosome=" + ChromosomeInfo.getString(chromosomeIndex) + " (index=" + chromosomeIndex + ") not in current GTB file");
    ...
    public void remove(String... chromosomes) {
        remove(ChromosomeInfo.getIndexes(chromosomes));
    }
    ...
    public <ChromosomeType> void remove(Collection<ChromosomeType> chromosomes) {
        remove(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public <ChromosomeType> void remove(Map<ChromosomeType, int[]> chromosomeNodeIndexListUnknownType) {
        HashMap<Integer, int[]> chromosomeNodeIndexList = ChromosomeInfo.identifyChromosome(chromosomeNodeIndexListUnknownType);
    ...
    public void retain(String... chromosomes) {
        retain(ChromosomeInfo.getIndexes(chromosomes));
    }
    ...
    public <ChromosomeType> void retain(Collection<ChromosomeType> chromosomes) {
        retain(ChromosomeInfo.identifyChromosome(chromosomes));
    }
    ...
    public <ChromosomeType> String chromosomeInfo(Collection<ChromosomeType> chromosomeIndexes) {
        return chromosomeInfo(ChromosomeInfo.identifyChromosome(chromosomeIndexes));
    }
