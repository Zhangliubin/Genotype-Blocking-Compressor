...
public class GTBNodes implements Iterable<GTBNode> {
    public final int chromosomeIndex;
    private SmartList<GTBNode> nodes;
    ...
    GTBNodes(GTBNode[] nodes) {
        this.chromosomeIndex = nodes[0].chromosomeIndex;
        this.nodes = new SmartList<>(nodes, true);
    }

    /**
     * 构造器方法
     * @param chromosomeIndex 当前节点群的染色体索引
     */
    public GTBNodes(int chromosomeIndex) {
        this(chromosomeIndex, 256);
    }
    ...
    public GTBNodes(String chromosome) {
        this(ChromosomeInfo.getIndex(chromosome));
    }
     ...
     */
    public GTBNodes(int chromosomeIndex, int capacity) {
        this.chromosomeIndex = chromosomeIndex;
        this.nodes = new SmartList<>(capacity, true);
    }
    ...
    public GTBNodes(String chromosome, int capacity) {
        this(ChromosomeInfo.getIndex(chromosome), capacity);
    }
    ...
    public void add(GTBNode node) {
        checkChromosome(node.chromosomeIndex);
        this.nodes.add(node);
    }
    ...
    public void add(GTBNodes otherNodes) {
        checkChromosome(otherNodes.chromosomeIndex);
        this.nodes.add(otherNodes.nodes);
    }
    ...
    public GTBNode[] get(int... nodeIndexes) {
        GTBNode[] out = new GTBNode[nodeIndexes.length];
        for (int i = 0; i < nodeIndexes.length; i++) {
            out[i] = get(nodeIndexes[i]);
        }
        return out;
    }
     ...
     * @param nodeIndexes 节点索引
     */
    public GTBNode[] get(Collection<Integer> nodeIndexes) {
        GTBNode[] out = new GTBNode[nodeIndexes.size()];
        int index = 0;
        for (int nodeIndex : nodeIndexes) {
            out[index++] = get(nodeIndex);
        }
        return out;
    }
     ...
     * @param nodeIndexes 节点的索引数组
     */
    public void remove(Collection<Integer> nodeIndexes) {
        // 再执行批量删除
        for (int index : nodeIndexes) {
            this.nodes.set(index, null);
        }
    ...
    public void retain(int... nodeIndexes) {
        this.nodes = new SmartList<>(get(nodeIndexes), true);
        flush(true);
    }
     ...
     * @param nodeIndexes 节点的索引数组
     */
    public void retain(Collection<Integer> nodeIndexes) {
        this.nodes = new SmartList<>(get(nodeIndexes), true);
        flush(true);
    }
    ...
    public boolean intersectPos(GTBNode node) {
        return ValueUtils.intersect(node.minPos, node.maxPos, get(0).minPos, get(-1).maxPos);
    }
    ...
    GTBNodes resetChromosome(int newChromosomeIndex) {
     ...
     */
    public void checkChromosome(int chromosomeIndex) {
        Assert.that(this.chromosomeIndex == chromosomeIndex, GBCExceptionOptions.GTBComponentException, "current node's chromosome is different from target's chromosome");
    }
    ...
    public String getRootInfo() {
        if (this.nodes.size() > 0) {
            return String.format("Chromosome %s: posRange=[%d, %d], numOfNodes=%d, numOfVariants=%d",
                    ChromosomeInfo.getString(chromosomeIndex), get(0).minPos, get(-1).maxPos, this.nodes.size(), numOfVariants());
        } else {
            return String.format("Chromosome %s: No. of nodes=0, No. of variants=0", ChromosomeInfo.getString(chromosomeIndex));
        }
    }
