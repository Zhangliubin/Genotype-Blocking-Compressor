    ...
    default byte encode(byte[] variant, int seek, int length) {
        ...
        if (length == 3) {
            ...
            if (length == 1) {
                ...
                throw new UnsupportedOperationException("The number of current variant’s alternative alleles is too large, " + getClass().getSimpleName() + " only supports a maximum of " + CoderConfig.MAX_ALLELE_NUM + ".");
            }
        }
    }
