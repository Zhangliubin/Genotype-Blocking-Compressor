...
public class VCFGenerator {
    ...
    static void generate(Prop prop, String chromosome, int variantNum, int subjectNum, boolean order) throws IOException {
        FileUtils.mkdir("./resource/simulate_vcf/");

        // 4 线程 bgzip 压缩
        FileStream vcf = new FileStream(outputDir + prop.name().toLowerCase() + "simu" + +variantNum + "V_" + subjectNum + "S.chr" + chromosome + ".vcf.gz", FileOptions.PARALLEL_BGZIP_WRITER_4);
...
//            }
            ...
            for (int variantNum : new int[]{1000, 5000, 10000, 50000, 100000, 500000, 1000000, 5000000}) {
                // 少量位点无序
                //generateUnsorted(dev.Prop.ZERO, "1", variantNum, 1000, 10000, 10, 5);

                // 大量位点无序
                generate(dev.Prop.RANDOM, "1", variantNum, 1000, false);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
